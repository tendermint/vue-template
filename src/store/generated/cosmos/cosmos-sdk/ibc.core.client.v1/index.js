	...
	actions: {
		...
		async QueryClientState(
			...
			try {
				let value = query
					? (
							await (await initQueryClient(rootGetters)).queryClientState(
								key.client_id,
								query
							)
					  ).data
		...
		async QueryClientStates(
			...
			try {
				let value = query
					? (
							await (await initQueryClient(rootGetters)).queryClientStates(
								query
							)
					  ).data
					: (await (await initQueryClient(rootGetters)).queryClientStates())
		...
		async QueryConsensusState(
			...
			try {
				let value = query
					? (
							await (await initQueryClient(rootGetters)).queryConsensusState(
								key.client_id,
								key.revision_number,
								key.revision_height,
								query
		...
		async QueryConsensusStates(
			...
			try {
				let value = query
					? (
							await (await initQueryClient(rootGetters)).queryConsensusStates(
								key.client_id,
								query
							)
					  ).data
		...
		async QueryClientParams(
			...
			try {
				let value = query
					? (
							await (await initQueryClient(rootGetters)).queryClientParams(
								query
							)
					  ).data
					: (await (await initQueryClient(rootGetters)).queryClientParams())
		...
		async sendMsgCreateClient({ rootGetters }, { value, fee, memo }) {
			try {
				const msg = await (await initTxClient(rootGetters)).msgCreateClient(
					value
				)
				const result = await (
		...
		async sendMsgUpgradeClient({ rootGetters }, { value, fee, memo }) {
			try {
				const msg = await (await initTxClient(rootGetters)).msgUpgradeClient(
					value
				)
				const result = await (
					await initTxClient(rootGetters)
		...
		async sendMsgUpdateClient({ rootGetters }, { value, fee, memo }) {
			try {
				const msg = await (await initTxClient(rootGetters)).msgUpdateClient(
					value
				)
				const result = await (
					await initTxClient(rootGetters)
