	...
	actions: {
		...
		async QueryEvidence(
			...
			try {
				let value = query
					? (
							await (await initQueryClient(rootGetters)).queryEvidence(
								key.evidence_hash,
								query
							)
					  ).data
		...
		async QueryAllEvidence(
			...
			try {
				let value = query
					? (await (await initQueryClient(rootGetters)).queryAllEvidence(query))
							.data
					: (await (await initQueryClient(rootGetters)).queryAllEvidence()).data
				while (all && value.pagination && value.pagination.nextKey != null) {
					let next_values = (
						await (await initQueryClient(rootGetters)).queryAllEvidence({
							...query,
		...
		async sendMsgSubmitEvidence({ rootGetters }, { value, fee, memo }) {
			try {
				const msg = await (await initTxClient(rootGetters)).msgSubmitEvidence(
					value
				)
				const result = await (
